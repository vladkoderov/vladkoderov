...
define( "DBFILE", realpath( str_replace( '\\', '/', __DIR__ ) ) . "/anonumgram.db" ); //Путь и имя файла с чатом
define( "REFRESHTIME", 5 * 1000 ); //Клиентская задержка опроса сервера
define( "HEADER", "anonumgram #1 chat" ); //Заголовок

define( "COOKIEPATH", "/" );

define( "CHATTRIM", 0 ); //Максимальная длина пересылаемого куска чата, 0 - без ограничений

define( "MAXUSERNAMELEN", 100 ); //Максимальная длина имени пользователя
define( "MAXUSERTEXTLEN", 10024 ); //Максимальная длина сообщения пользователя
...
if ( $mode == "post" ) {
	...
	$msg = '<div class="msg"><div class="info"><span class="name">' . $name . '</span><span class="misc"><span class="date">' . date( "d.m.Y H:i:s" ) . '</span> </span></span></div>' . $text . '</div>' . "\n\n";
